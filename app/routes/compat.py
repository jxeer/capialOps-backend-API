...
@compat_bp.route("/global-seed", methods=["POST"])
def global_seed():
    """Create global seed data that ALL users can see (for demo purposes).
    This ignores user filtering - everyone sees the same demo data.
    """
    ...
    try:
        ...
        # Create portfolio (no user_id for global access)
        portfolio = Portfolio(
            name="CapitalOps Premier Fund",
            description="Diversified real estate development portfolio",
        )
        ...
        # Create assets
        assets = [
            Asset(portfolio_id=portfolio.id, name="The Meridian", location="Austin, TX", asset_type="Multifamily", square_footage=185000, status="Active", asset_manager="Sarah Chen"),
            Asset(portfolio_id=portfolio.id, name="Parkside Commons", location="Denver, CO", asset_type="Mixed-Use", square_footage=120000, status="Pre-dev", asset_manager="Sarah Chen"),
            Asset(portfolio_id=portfolio.id, name="Harbor Point", location="Miami, FL", asset_type="Commercial", square_footage=95000, status="Active", asset_manager="Mike Torres"),
        ]
        ...
        return jsonify({
            "message": "Global seed data created - visible to all users",
        ...
        import logging
        logging.error(f"Global seed error: {str(e)}")
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
